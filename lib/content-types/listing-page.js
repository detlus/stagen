...
ListingPage.prototype.getContents = function(type, options) {
  ...
  if (!this.contents || options.reset) {
    ...
    this.contents = this.content_manager.getContentFiles(type, {filter: {published: true}});
  }
};
...
ListingPage.prototype.isValid = function() {
  if (this.data && this.data.listing_item && this.data.listing_item.type && this.data.permalink) {
    return true;
  }
  else {
    return false;
