...
ListingPage.prototype.generate = function() {
  ...
  for (let items of this) {
    ...
    var page_template_file = that.content_manager.site.getThemeDir() + '/_listings/' + that.data.listing_item.type + 's' + '.twig';
    try{
      fs.statSync(page_template_file);
    }
    catch (e){
      throw new Error("The listing template file " + that.data.listing_item.type + " does not exist in theme " + this.site.configuration.theme);
    }
