...
Post.prototype.render = function(mode) {
  ...
  if (!Post.type_templates['post']) {
    type_template = this.content_manager.site.getThemeDir() + '/_types/' + this.type + '.twig';
    try{
      fs.statSync(type_template);
    }
    catch (e){
      throw new Error("The type template file " + this.type + " does not exist in theme " + this.content_manager.site.configuration.theme);
    }
