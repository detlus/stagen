...
ListingPage.prototype.generate = function() {
  ...
  for (let items of this) {
    ...
    _.each(items, function(item) {
      var post = new stagen.Post(that.content_manager, that.content_manager.getContentData(item.filename))
      template_var_items.push(post.getData());
    });
    ...
    var content_html = template.render({
      listing: {
    ...
    var page_template_file = that.content_manager.site.getThemeDir() + '/_layouts/' + that.page_data.attributes.layout + '.twig';
    try{
    ...
    var html = page_template.render({
      page: _.extend({content: content_html}, that.page_data.attributes),
