...
ContentManager.prototype.generateOutputForDependents = function(filename) {
  var that = this;
  if (this.dependency_map[filename]) {
    _.each(this.dependency_map[filename], function(dependent){
      that.generateSingleOutput(dependent);
    });
  }
};
...
ContentManager.prototype.generateSingleOutput = function(filename) {
  if (this.generated_contents.indexOf(filename) > -1) {
  ...
  if (content) {
    try {
      ...
      this.generateOutputForDependents(filename);
