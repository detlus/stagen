...
ListingPage.prototype.generate = function() {
  ...
  for (let items of this) {
    ...
    var page_template_file = that.content_manager.site.getThemeDir() + '/_types/' + that.type + '.twig';
    try{
      fs.statSync(page_template_file);
    }
    catch (e){
      throw new Error("The listing template file " + that.type + " does not exist in theme " + this.content_manager.site.configuration.theme);
    }
