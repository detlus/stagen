...
ContentManager.prototype.scanFiles = function(type, options) {
  ...
  if (_.isUndefined(ContentManager.cache[type]) || options.reset) {
    ...
    _.each(filesinfo_temp, function(fileinfo, index) {
      ...
      // TODO: Make file attributes default values configurable.
      // For example, here published property default value 'true' is harcoded.
      fileinfo = _.extend(
        fileinfo,
        _.defaults(_.pick(fm_data.attributes, that.site.configuration.content_indexs), {published: true})
      );
    ...
    ContentManager.cache[type] = filesinfo;
  }
  else {
    filesinfo = ContentManager.cache[type];
  }
  return filesinfo;
};

/**
 * Retrieve list of contents based on criteria if provided.
 */
ContentManager.prototype.getContentFiles = function(type, options) {
  // Get all of the content of given type.
  var filesinfo =  this.scanFiles(type);
  options = _.defaults(options || {}, {filter: {}});

  var filesinfo_filtered = [];
  var unwanted_fileinfo_index = [];
  // If filtering criteria are specified,
  // then prepare list of files to be removed.
  if (!_.isEmpty(options.filter)) {
    _.each(filesinfo, function(fileinfo, index) {
      _.each(options.filter, function(value, property) {
        if (fileinfo[property] !== value) {
          unwanted_fileinfo_index.push(index);
        }
      });
    });
  }
  // Remove marked files from the list.
  _.each(filesinfo, function(fileinfo, index) {
    if (unwanted_fileinfo_index.indexOf(index) === -1) {
      filesinfo_filtered.push(fileinfo);
    }
  });
  return filesinfo_filtered;
};
