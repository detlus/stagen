...
ContentManager.prototype.generateSingleOutput = function(filename) {
  ...
  if (content.isValid()) {
    try {
      ...
      this.site.addError("Content generation failed for " + content.filename, e);
    }
  }
  else {
    this.site.addError("Invalid content " + content.filename);
  }
};
    ...
    _.each(contents, function(content){
        if (content.isValid()) {
          try {
            ...
            that.site.addError("Content generation failed for " + content.filename, e);
          }
        }
        else {
