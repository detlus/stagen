...
class ContentManager {
  ...
  loadContent(filename, reset) {
    if (reset || !this.content_cache[filename]) {
      ...
      if (this.type_classes[type]) {
        ...
        this.site.addError(utils.format("Handler for content type '%s' does not exist.", type));
      }
    }
  ...
  /**
   * Discover content type classes from give directory
   * @param {string} directory 
   */
  async discoverTypes(directory) {
    const dir = new URL(directory); // example directory
    ...
    for (const file of readdirSync(dir)) {
      ...
      this.registerType(type.default);
    }
  }
}
